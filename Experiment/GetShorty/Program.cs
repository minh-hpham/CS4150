...
namespace GetShorty
{
    ...
    class Dijkstra
    {
        struct edge
        {
            ...
            public edge(int to, float weight) : this()
            {
                this.to = to;
                this.weight = weight;
            }
        ...
        Dictionary<int, LinkedList<edge>> nodes = new Dictionary<int, LinkedList<edge>>();
        int[] dist;
        int[] prev;
        int size;
        public Dijkstra(int s)
        {
            ...
            for (int i = 0; i < s; i++)
            {
                nodes.Add(i, new LinkedList<edge>());       
            }            
            size = s;
        }
        public void addEdge(int first, int second, float f)
        {
            nodes[first].AddLast(new edge(second,f));
            nodes[second].AddLast(new edge(first, f));
        }
        ...
        public int[] shortestReach(int startId)
        {
           ...
           dist[startId] = 1;
            ...
            q.insertOrChanges(startId,1);

            while (q.Count() != 0)
            {
                int u = q.Dequeue();
                foreach (edge v in nodes[u])
                {
    ...
    public class PriorityQueue<TPriority, TValue> : ICollection<KeyValuePair<TPriority, TValue>>
    {
        ...
        internal void insertOrChanges(int startId, int v)
        {
